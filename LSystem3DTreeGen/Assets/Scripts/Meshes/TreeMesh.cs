...
public class TreeMesh : MonoBehaviour
{
    ...
    public CreateTreeMesh.MeshInfo Init(List<(Transform,bool)> turtles)
    {
        vertices = new List<Vector3>();
        gs = new List<GameObject>();
        vs = new List<(Vector3, Transform)>();

        for (int i = 0; i < turtles.Count - 1; i++)
        {
            if(turtles[i+1].Item2 == true)
            {
                i += 1;
                continue;
            }
            Vector3 pos = turtles[i + 1].Item1.transform.position - (turtles[i].Item1.transform.up);
            Quaternion rot = turtles[i].Item1.transform.rotation;
            Vector3 pos2 = turtles[i + 1].Item1.transform.position + (turtles[i + 1].Item1.transform.up);
            Quaternion rot2 = turtles[i + 1].Item1.transform.rotation;
        ...
        CreateTreeMesh.MeshInfo mInfo = new CreateTreeMesh.MeshInfo();
        mInfo.vertices = vertices.ToArray();
        mInfo.triangles = CreateTriangles(vertices.ToArray());
    ...
    List<Vector3> CreateCircleAroundPoint(Transform transform, int nbPointsInCircle, float width)
    {
        ...
        Vector3 initPosition = Vector3.Cross(transform.right, up) * .02f;
